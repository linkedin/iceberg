...
class RowFilterValueReader implements OrcRowReader<Object[]> {
  ...
  RowFilterValueReader(TypeDescription readSchema, TypeDescription filterSchema) {
    ...
    filterSchemaToReadSchemaColumnIndex =
        buildFilterSchemaToReadSchemaColumnIndex(readSchema, filterSchema);
  }

  private int[] buildFilterSchemaToReadSchemaColumnIndex(
      TypeDescription readSchema, TypeDescription filterSchema) {
        ...
        switch (longType) {
          ...
          default:
            throw new IllegalStateException(
                "Unhandled Long type found in ORC type attribute: " + longType);
        }
      case STRING:
      case CHAR:
      case VARCHAR:
