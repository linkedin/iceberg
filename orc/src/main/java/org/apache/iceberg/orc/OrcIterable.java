...
class OrcIterable<T> extends CloseableGroup implements CloseableIterable<T> {
  ...
  public CloseableIterator<T> iterator() {
    ...
    // to push down filters to ORC's SearchArgument, since we are not reading anything from files at
    // all
    ...
    // underlying ORC files, since the Iceberg schema has a different representation than the ORC
    // schema.
    ...
    if (rowFilter == null) {
      ...
      if (batchReaderFunction != null) {
        OrcBatchReader<T> batchReader =
            (OrcBatchReader<T>) batchReaderFunction.apply(readOrcSchema);
        ...
        return new OrcRowIterator<>(
            rowBatchIterator, (OrcRowReader<T>) readerFunction.apply(readOrcSchema), null, null);
      }
    } else {
      Preconditions.checkArgument(
          batchReaderFunction == null, "Row-level filtering not supported by vectorized reader");
      Set<Integer> filterColumnIds = TypeUtil.getProjectedIds(rowFilter.requiredSchema());
      Set<Integer> filterColumnIdsNotInReadSchema =
          Sets.difference(filterColumnIds, TypeUtil.getProjectedIds(schema));
      Schema extraFilterColumns =
          TypeUtil.select(rowFilter.requiredSchema(), filterColumnIdsNotInReadSchema);
      Schema finalReadSchema = TypeUtil.join(schema, extraFilterColumns);

      TypeDescription finalReadOrcSchema =
          ORCSchemaUtil.buildOrcProjection(finalReadSchema, fileSchemaWithIds);
      TypeDescription rowFilterOrcSchema =
          ORCSchemaUtil.buildOrcProjection(rowFilter.requiredSchema(), fileSchemaWithIds);
      RowFilterValueReader filterReader =
          new RowFilterValueReader(finalReadOrcSchema, rowFilterOrcSchema);

      return new OrcRowIterator<>(
          newOrcIterator(
              file, finalReadOrcSchema, start, length, orcFileReader, sarg, recordsPerBatch),
          (OrcRowReader<T>) readerFunction.apply(readOrcSchema),
          rowFilter,
          filterReader);
    }
  }
  ...
  private static class OrcRowIterator<T> implements CloseableIterator<T> {
    ...
    OrcRowIterator(
        VectorizedRowBatchIterator batchIter,
        OrcRowReader<T> reader,
        OrcRowFilter filter,
        RowFilterValueReader filterReader) {
