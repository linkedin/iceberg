      ...
      if (avroField != null) {
        ...
        if (field.hasDefaultValue() && !AvroSchemaUtil.hasNonNullDefaultValue(avroField)) {
          ...
          Schema.Field newField =
              new Schema.Field(
                  avroField.name(), newFiledSchema, avroField.doc(), field.getDefaultValue());
          newField.addProp(AvroSchemaUtil.FIELD_ID_PROP, field.fieldId());
          updatedFields.add(newField);
          hasChange = true;
        } else {
          // otherwise (i.e., expectedFiled has no default value, or it is null) we can use
          // avroField as is
        ...
        // then default to null and assign a unique suffix to the field to make sure that even if
        // records in the
