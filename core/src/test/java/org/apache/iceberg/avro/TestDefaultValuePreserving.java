...
public class TestDefaultValuePreserving {
  ...
  public void testSchemaToTypeRecord() {
    ...
    List<String> names =
        recordSchema.getFields().stream().map(Schema.Field::name).collect(Collectors.toList());
    ...
    Assert.assertEquals(
        defaultValue, record.asStructType().field(fieldWithDefaultName).getDefaultValue());
  }
  ...
  public void testTypeToSchemaStruct() {
    List<Types.NestedField> nestedFields =
        ImmutableList.of(
            ...
            Types.NestedField.required(
                1, fieldWithDefaultName, Types.IntegerType.get(), defaultValue, null));
    ...
    for (int i = 0; i < nestedFields.size(); i++) {
      if (nestedFields.get(i).hasDefaultValue()) {
        ...
        Assert.assertEquals(
            nestedFields.get(i).getDefaultValue(), structSchema.getFields().get(i).defaultVal());
      } else {
