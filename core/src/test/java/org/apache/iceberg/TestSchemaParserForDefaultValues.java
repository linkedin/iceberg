...
public class TestSchemaParserForDefaultValues {
  ...
  private void assertEqualStructs(
      org.apache.iceberg.Schema expected, org.apache.iceberg.Schema actual) {
    if (expected == null) {
      ...
      if (expectedField.hasDefaultValue()) {
        ...
        switch (expectedField.type().typeId()) {
          ...
          case FIXED:
            Assert.assertTrue(
                Arrays.equals(
                    (byte[]) expectedField.getDefaultValue(),
                    (byte[]) actualField.getDefaultValue()));
            break;
          default:
  ...
  private void testToFromJsonPreservingDefaultValues(
      String[] fieldNames, Schema[] fieldsSchemas, Object[] defaults) {
    List<Field> fields = Lists.newArrayList();
    IntStream.range(0, defaults.length)
        .forEach(
  ...
  public void testLogicalTypes() {
    ...
    String[] fieldNames = {"dateField", "timeField", "timestampField", "uuidField", "decimalField"};

    Object[] defaults = {Integer.valueOf(123446), longDefault, "randomUUID", longDefault};
    ...
    Schema[] logicals = {dateSchema, timestampSchema, uuidSchema, bigDecimalSchema};
  ...
  public void testNestedTypes() {
    ...
    Map<String, Object> defaultStruct =
        ImmutableMap.of(
            structStringFieldName, "default string", structBooleanFieldName, Boolean.TRUE);
    List<Integer> defaultList = Arrays.asList(1, 2);
    Map<String, Long> defaultMap =
        ImmutableMap.of("key1", Long.valueOf(1L), "key2", Long.valueOf(2L));
  ...
  public void testNestedOfNestedWithDefault() {
    ...
    List<Schema.Field> structFields =
        ImmutableList.of(
            ...
            new Schema.Field(
                structMapFieldName, Schema.createMap(Schema.create(LONG)), null, defaultMap));

    String[] fieldNames = {"intFieldNoDefault", "structFieldNoDefault"};
    Schema[] topLevelFields = {
      Schema.create(INT), Schema.createRecord("name", null, "namespace", false, structFields)
    };

    List<Schema.Field> fields = Lists.newArrayList();
    IntStream.range(0, fieldNames.length)
        .forEach(i -> fields.add(new Schema.Field(fieldNames[i], topLevelFields[i], null)));
  ...
  public void testDeepNestedWithDefault() {
    ...
    List<Schema.Field> intermediateStructFields =
        ImmutableList.of(
    ...
    Schema intermediateStruct =
        Schema.createRecord("name", null, "namespace1", false, intermediateStructFields);
    String[] fieldNames = {"topLevelLong", "topLevelString", "topLevelStruct"};
    Schema[] topLevelFields = {Schema.create(LONG), Schema.create(STRING), intermediateStruct};
