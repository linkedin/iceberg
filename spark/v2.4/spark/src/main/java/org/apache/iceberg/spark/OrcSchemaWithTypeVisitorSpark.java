...
public abstract class OrcSchemaWithTypeVisitorSpark<T> extends OrcSchemaWithTypeVisitor<T> {
    ...
    for (int i = 0, j = 0; i < iFields.size(); i++) {
      ...
      if (field == null || (iField.fieldId() != ORCSchemaUtil.fieldId(field))) {
        ...
        if (MetadataColumns.nonMetadataColumn(iField.name())
          ...
          idToConstant.put(
              iField.fieldId(),
              BaseDataReader.convertConstant(iField.type(), iField.getDefaultValue()));
        }
      } else {
  ...
  private static boolean icebergFiledIdsContainOrcFieldIdsInOrder(
  ...
  /**
   * Checks whether the first list contains all the integers in the same order as regarding to the
   * second list, the first list can contain extra integers that the second list doesn't, but the
   * ones that exist in the second list should occur in the same relative order in the first list.
   *
