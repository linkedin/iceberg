...
public abstract class InStream extends InputStream {
  ...
  protected void reset(DiskRangeList input) {
    bytes = input;
    while (input != null && (input.getEnd() <= offset || input.getOffset() > offset + length)) {
      input = input.next;
    }
  ...
  public static class UncompressedStream extends InStream {
    ...
    /**
     * Create the stream without calling reset on it. This is used for the subclass that needs to do
     * more setup.
     *
    ...
    public String toString() {
      return "uncompressed stream "
          ...
          + length
          + " range: "
          + getRangeNumber(bytes, currentRange)
          + " offset: "
  ...
  static class EncryptionState {
    ...
    /**
     * We are seeking to a new range, so update the cipher to change the IV to match. This code
     * assumes that we only support encryption in CTR mode.
     *
    ...
    /**
     * Decrypt the given range into the decrypted buffer. It is assumed that the cipher is correctly
     * initialized by changeIv before this is called.
     *
    ...
    ByteBuffer decrypt(ByteBuffer encrypted) {
      ...
      try {
        ...
        if (output != length) {
          throw new IllegalArgumentException("Problem decrypting " + name + " at " + offset);
        }
      } catch (ShortBufferException e) {
        throw new IllegalArgumentException("Problem decrypting " + name + " at " + offset, e);
      }
      decrypted.flip();
      return decrypted;
    }
  ...
  public static class EncryptedStream extends UncompressedStream {
    ...
    public EncryptedStream(
        Object name, DiskRangeList input, long offset, long length, StreamOptions options) {
      super(name, offset, length);
  ...
  public static class CompressedStream extends InStream {
    ...
    /**
     * Create the stream without resetting the input stream. This is used in subclasses so they can
     * finish initializing before reset is called.
     *
    ...
    protected void setCurrent(DiskRangeList newRange, boolean isJump) {
      ...
      if (newRange != null) {
        ...
        compressed.limit(pos + (int) Math.min(compressed.remaining(), length - position));
      }
    }

    private int readHeaderByte() {
      while (currentRange != null && (compressed == null || compressed.remaining() <= 0)) {
        setCurrent(currentRange.next, false);
      }
    ...
    boolean seek(long desired) throws IOException {
      ...
      for (DiskRangeList range = bytes; range != null; range = range.next) {
        if (range.getOffset() <= posn
            && (range.next == null ? posn <= range.getEnd() : posn < range.getEnd())) {
          position = desired;
    ...
    public String toString() {
      return "compressed stream "
          ...
          + getRangeNumber(bytes, currentRange)
          + " offset: "
          + (compressed == null ? 0 : compressed.position())
          + " limit: "
          + (compressed == null ? 0 : compressed.limit())
          + rangeString()
          + (uncompressed == null
              ? ""
  ...
  private static class EncryptedCompressedStream extends CompressedStream {
    ...
    protected void setCurrent(DiskRangeList newRange, boolean isJump) {
      ...
      if (newRange != null) {
        ...
        encrypted.limit(ignoreBytes + (int) Math.min(encrypted.remaining(), length));
   ...
   */
  public static InStream create(
      Object name, DiskRangeList input, long offset, long length, StreamOptions options) {
    LOG.debug("Reading {} with {} from {} for {}", name, options, offset, length);
