...
public class TestReadOrcFileWithoutIDs {
  ...
  private static final Types.StructType SUPPORTED_PRIMITIVES =
      Types.StructType.of(
          ...
          //      Disabled some primitives because they cannot work without Iceberg's type
          // attributes and hence won't be present
          //      in old data anyway
  ...
  public void writeAndValidateORCFileWithoutIds() throws IOException {
    ...
    Assert.assertEquals(
        "Ensure written file does not have IDs in the file schema",
        0,
  ...
  private static TypeDescription orcFileSchema(File file) throws IOException {
    return OrcFile.createReader(
            new Path(file.getPath()), OrcFile.readerOptions(new Configuration()))
        .getSchema();
  }
  ...
  private static class OrcWriter implements Closeable {
    ...
    private OrcWriter(Schema schema, File file) {
      ...
      OrcFile.WriterOptions options =
          OrcFile.writerOptions(new Configuration()).useUTCTimestamp(true);
