...
public class TestOrcRowLevelFiltering {
  ...
  public void testReadOrcWithRowFilterProjection() throws IOException {
    Schema projectedSchema = new Schema(required(101, "data1", Types.StringType.get()));

    List<Record> expected =
        RECORDS.subList(75, 100).stream()
            .map(
                r -> {
  ...
  private void testReadOrcWithRowFilter(
      Schema schema, OrcRowFilter rowFilter, List<Record> expected) throws IOException {
  ...
  private OrcRowFilter rowFilterId() {
    return new OrcRowFilter() {
      ...
      public Schema requiredSchema() {
        return new Schema(required(100, "id", Types.LongType.get()));
      }
  ...
  private OrcRowFilter rowFilterIdAndData1() {
    return new OrcRowFilter() {
      @Override
      public Schema requiredSchema() {
        return new Schema(SCHEMA.findField("id"), SCHEMA.findField("data1"));
      }
  ...
  private OrcRowFilter rowFilterData3() {
    return new OrcRowFilter() {
      @Override
      public Schema requiredSchema() {
        return new Schema(optional(104, "data3", Types.LongType.get()));
      }
