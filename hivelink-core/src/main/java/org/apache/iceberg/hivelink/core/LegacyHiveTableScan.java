...
/**
 * A {@link DataTableScan} which uses Hive table and partition metadata to read tables. This scan
 * does not provide any time travel, snapshot isolation, incremental computation benefits.
 */
public class LegacyHiveTableScan extends DataTableScan {
  ...
  public CloseableIterable<FileScanTask> planFiles() {
    ...
    Listeners.notifyAll(new ScanEvent(table().toString(), -1, filter(), schema()));
    ...
    Iterable<Iterable<FileScanTask>> tasks =
        Iterables.transform(
                ...
                Iterables.transform(
                    ...
                    file ->
                        new BaseFileScanTask(
                            file, new DeleteFile[0], schemaString, specString, residuals)));
