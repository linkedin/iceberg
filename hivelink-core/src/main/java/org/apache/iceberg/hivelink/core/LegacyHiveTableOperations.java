...
public class LegacyHiveTableOperations extends BaseMetastoreTableOperations {
  ...
  protected LegacyHiveTableOperations(
      Configuration conf, ClientPool metaClients, String database, String table) {
    this.conf = conf;
  ...
  protected void doRefresh() {
    try {
      ...
      Map<String, String> tableProperties =
          Maps.newHashMap(LegacyHiveTableUtils.getTableProperties(hiveTable));
      ...
      TableMetadata metadata =
          newTableMetadataWithoutFreshIds(
              schema, spec, hiveTable.getSd().getLocation(), tableProperties);
      setCurrentMetadata(metadata);
    } catch (TException e) {
      String errMsg =
          String.format("Failed to get table info from metastore %s.%s", databaseName, tableName);
  ...
  /**
   * Returns an {@link Iterable} of {@link Iterable}s of {@link DataFile}s which belong to the
   * current table and match the partition predicates from the given expression.
   *
   * <p>Each element in the outer {@link Iterable} maps to an {@link Iterable} of {@link DataFile}s
   * originating from the same directory
   */
  Iterable<Iterable<DataFile>> getFilesByFilter(Expression expression) {
    ...
    // Note that we return an Iterable of Iterables here so that the TableScan can process iterables
    // of individual
    ...
    return Iterables.transform(
        ...
        directory -> {
          ...
          return Iterables.transform(
              ...
              file ->
                  createDataFile(
                      file, current().spec(), directory.partitionData(), directory.format()));
        });
  }

  private DirectoryInfo getDirectoryInfo() {
    Preconditions.checkArgument(
        current().spec().fields().isEmpty(),
    ...
    try {
      ...
      String errMsg =
          String.format(
              "Failed to get table info for %s.%s from metastore", databaseName, tableName);
      throw new RuntimeException(errMsg, e);
    } catch (InterruptedException e) {
      Thread.currentThread().interrupt();
  ...
  private List<DirectoryInfo> getDirectoryInfosByFilter(Expression expression) {
    ...
    try {
      LOG.info(
          "Fetching partitions for {}.{} with expression: {}", databaseName, tableName, expression);
      Set<String> partitionColumnNames =
          current().spec().identitySourceIds().stream()
              .map(id -> current().schema().findColumnName(id))
              .collect(Collectors.toSet());
      Expression simplified =
          HiveExpressions.simplifyPartitionFilter(expression, partitionColumnNames);
      ...
      if (simplified.equals(Expressions.alwaysFalse())) {
        // If simplifyPartitionFilter returns FALSE, no partitions are going to match the filter
        // expression
        partitions = ImmutableList.of();
      } else if (simplified.equals(Expressions.alwaysTrue())) {
        // If simplifyPartitionFilter returns TRUE, all partitions are going to match the filter
        // expression
        partitions =
            metaClients.run(
        ...
        LOG.info(
            "Listing partitions for {}.{} with filter string: {}",
            databaseName,
            tableName,
        ...
        try {
              ...
              metaClients.run(
                  client ->
                      client.listPartitionsByFilter(
                          databaseName, tableName, partitionFilterString, (short) -1));
          ...
          List<Partition> allPartitions =
              metaClients.run(
                  client ->
                      client.listPartitionsByFilter(databaseName, tableName, null, (short) -1));
          partitions =
                      ...
                      partition -> {
                        ...
                        for (int i = 0; i < record.size(); i++) {
                          ...
                          switch (partitionSchema.fields().get(i).type().typeId()) {
                            case DATE:
                              record.set(i, (int) LocalDate.parse(value).toEpochDay());
                              break;
                            case TIMESTAMP:
                              // This format seems to be matching the hive timestamp column
                              // partition string literal value
                              record.set(
                                  i,
                                  LocalDateTime.parse(
                                              value,
                                              new DateTimeFormatterBuilder()
                                                  ...
                                                  .appendFraction(
                                                      ChronoField.NANO_OF_SECOND, 0, 9, true)
                                                  .toFormatter())
      ...
      String errMsg =
          String.format(
              "Failed to get partition info for %s.%s + expression %s from metastore",
  ...
  public void commit(TableMetadata base, TableMetadata metadata) {
    throw new UnsupportedOperationException(
        "Writes not supported for Hive tables without Iceberg metadata");
  }
  ...
  private TableMetadata newTableMetadataWithoutFreshIds(
