...
/**
 * A {@link HiveSchemaWithPartnerVisitor} which augments a Hive schema with extra metadata from a
 * partner Avro schema and generates a resultant "merged" Avro schema
 *
 * <p>1. Fields are matched between Hive and Avro schemas using a case insensitive search by field
 * name 2. Copies field names, nullability, default value, field props from the Avro schema 3.
 * Copies field type from the Hive schema. TODO: We should also handle some cases of type promotion
 * where the types in Avro are potentially more correct e.g.BINARY in Hive to FIXED in Avro, STRING
 * in Hive to ENUM in Avro, etc 4. Retains fields found only in the Hive schema; Ignores fields
 * found only in the Avro schema 5. Fields found only in Hive schema are represented as optional
 * fields in the resultant Avro schema 6. For fields found only in Hive schema, field names are
 * sanitized to make them compatible with Avro identifier spec
 */
public class MergeHiveSchemaWithAvro
    extends HiveSchemaWithPartnerVisitor<Schema, Schema.Field, Schema, Schema.Field> {

  public static Schema visit(StructTypeInfo typeInfo, Schema schema) {
    return visit(typeInfo, schema, new MergeHiveSchemaWithAvro(), AvroPartnerAccessor.INSTANCE);
  }
  ...
  public Schema struct(StructTypeInfo struct, Schema partner, List<Schema.Field> fieldResults) {
    ...
    if (partner == null || extractIfOption(partner).getType() != Schema.Type.RECORD) {
      ...
      result =
          Schema.createRecord(
              "record" + recordNum, null, "namespace" + recordNum, false, fieldResults);
    } else {
  ...
  public Schema.Field field(String name, TypeInfo field, Schema.Field partner, Schema fieldResult) {
    // No need to infer `shouldResultBeOptional`. We expect other visitor methods to return optional
    // schemas
    ...
    if (partner == null) {
      ...
      return new Schema.Field(
          AvroSchemaUtil.makeCompatibleName(name),
          fieldResult,
          null,
          Schema.Field.NULL_DEFAULT_VALUE);
    } else {
      // TODO: How to ensure that field default value is compatible with new field type generated
      // from Hive?
      // Copy field type from the visitor result, copy everything else from the partner
      // Avro requires the default value to match the first type in the option, reorder option if
      // required
  ...
  /**
   * Reorders an option schema so that the type of the provided default value is the first type in
   * the option schema
   *
   * <p>e.g. If the schema is (NULL, INT) and the default value is 1, the returned schema is (INT,
   * NULL) If the schema is not an option schema or if there is no default value, schema is returned
   * as-is
   */
  ...
  public Schema list(ListTypeInfo list, Schema partner, Schema elementResult) {
    // if there was no matching Avro list, or if matching Avro list was an option, return an
    // optional list
    boolean shouldResultBeOptional = partner == null || AvroSchemaUtil.isOptionSchema(partner);
  ...
  public Schema map(MapTypeInfo map, Schema partner, Schema keyResult, Schema valueResult) {
    Preconditions.checkArgument(
        extractIfOption(keyResult).getType() == Schema.Type.STRING,
        "Map keys should always be non-nullable strings. Found: %s",
        keyResult);
    // if there was no matching Avro map, or if matching Avro map was an option, return an optional
    // map
    boolean shouldResultBeOptional = partner == null || AvroSchemaUtil.isOptionSchema(partner);
  ...
  private Schema checkCompatibilityAndPromote(Schema schema, Schema partner) {
    ...
    //       Also do type promotion if required, schema = string & partner = enum, schema = bytes &
    // partner = fixed, etc
    return schema;
  }
  ...
  /**
   * A {@link PartnerAccessor} which matches the requested field from a partner Avro struct by case
   * insensitive field name match
   */
  private static class AvroPartnerAccessor implements PartnerAccessor<Schema, Schema.Field> {
    ...
    public Schema.Field fieldPartner(Schema partner, String fieldName) {
      ...
      return (schema.getType() == Schema.Type.RECORD)
          ? findCaseInsensitive(schema, fieldName)
          : null;
    }
    ...
    private Schema.Field findCaseInsensitive(Schema struct, String fieldName) {
      ...
      // TODO: Optimize? This will be called for every struct field, we will run the for loop for
      // every struct field
  ...
  public Schema hivePrimitiveToAvro(PrimitiveTypeInfo primitive) {
    switch (primitive.getPrimitiveCategory()) {
      ...
      case DECIMAL:
        ...
        return LogicalTypes.decimal(dti.getPrecision(), dti.getScale())
