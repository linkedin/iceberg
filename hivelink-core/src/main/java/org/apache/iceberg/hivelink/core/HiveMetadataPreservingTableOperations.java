...
/**
 * A {@link HiveTableOperations} that does not override any existing Hive metadata. TODO: This
 * extension should be removed once dual-publish of iceberg+hive is stopped.
 *
 * <p>The behaviour of this class differs from {@link HiveTableOperations} in the following ways: 1.
 * Does not modify serde information of existing Hive table, this means that if Iceberg schema is
 * updated Hive schema will remain stale 2. If the Hive table already exists, no error is thrown.
 * Instead Iceberg metadata is added to the table
 *
 * <p>This behaviour is useful if the Iceberg metadata is being generated/updated in response to
 * Hive metadata being updated.
 */
public class HiveMetadataPreservingTableOperations extends HiveTableOperations {
  ...
  private static final Logger LOG =
      LoggerFactory.getLogger(HiveMetadataPreservingTableOperations.class);
  ...
  protected HiveMetadataPreservingTableOperations(
      Configuration conf,
  ...
  protected void doRefresh() {
    ...
    try {
      ...
      if (tableType == null || !tableType.equalsIgnoreCase(ICEBERG_TABLE_TYPE_VALUE)) {
        // [LINKEDIN] If table type is not Iceberg, that means there is no Iceberg metadata for the
        // table yet.
        // So do not throw an error, instead just continue, currentMetadata will continue to remain
        // null
        // which is what doRefresh would do if the table did not exist and further operations should
        // work correctly
        ...
        if (metadataLocation == null) {
          String errMsg =
              String.format(
                  "%s.%s is missing %s property", database, tableName, METADATA_LOCATION_PROP);
        ...
        if (!io().newInputFile(metadataLocation).exists()) {
          String errMsg =
              String.format(
                  "%s property for %s.%s points to a non-existent file %s",
  ...
  protected void doCommit(TableMetadata base, TableMetadata metadata) {
    ...
    try {
      ...
      // [LINKEDIN] Do not touch the Hive schema of the table, just modify Iceberg specific
      // properties
      ...
      if (e.getMessage().contains("Table/View 'HIVE_LOCKS' does not exist")) {
        throw new RuntimeException(
            "Failed to acquire locks from metastore because 'HIVE_LOCKS' doesn't "
  ...
  /**
   * [LINKEDIN] Due to an issue that the table read in is sometimes corrupted and has incorrect
   * columns, compare the table columns to the avro.schema.literal property (if it exists) and fix
   * the table columns if there is a mismatch
   *
  ...
  static boolean fixMismatchedSchema(Table table) {
    ...
    if (table.getSd().getCols().size() != hiveCols.size()) {
      ...
      Map<String, String> hiveFieldMap =
          hiveCols.stream()
              .collect(
                  Collectors.toMap(
                      field -> field.getName().toLowerCase(),
                      field -> field.getType().toLowerCase()));
    ...
    if (schemaMismatched) {
      LOG.warn(
          "Schema columns don't match avro.schema.literal, setting columns to avro.schema.literal. Schema "
      ...
      if (!Strings.isNullOrEmpty(table.getSd().getInputFormat())
          && table.getSd().getInputFormat().contains("OrcInputFormat")) {
  ...
  private static void updateORCStorageDesc(List<FieldSchema> hiveCols, Table table) {
    String columnsString =
        hiveCols.stream().map(FieldSchema::getName).collect(Collectors.joining(","));
    String typesString =
        hiveCols.stream().map(FieldSchema::getType).collect(Collectors.joining(","));
