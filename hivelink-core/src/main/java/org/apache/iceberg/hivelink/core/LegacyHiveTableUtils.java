...
class LegacyHiveTableUtils {
  ...
  static Schema getSchema(org.apache.hadoop.hive.metastore.api.Table table) {
    ...
    org.apache.avro.Schema avroSchema =
        schemaStr != null
            ? new org.apache.avro.Schema.Parser().setValidateDefaults(false).parse(schemaStr)
            : null;
    Schema schema;
    if (avroSchema != null) {
      ...
      if (serde.equals("org.apache.hadoop.hive.serde2.avro.AvroSerDe")
        ...
        // Case 1: If serde == AVRO, early escape; Hive column info is not reliable and can be empty
        // for these tables
        //         Hive itself uses avro.schema.literal as source of truth for these tables, so this
        // should be fine
        // Case 2: If avro.schema.literal has duplicate column names when lowercased, that means we
        // cannot do reliable
        //         matching with Hive schema as multiple Avro fields can map to the same Hive field
        finalAvroSchema = avroSchema;
      } else {
        finalAvroSchema =
            MergeHiveSchemaWithAvro.visit(
                structTypeInfoFromCols(table.getSd().getCols()), avroSchema);
      }
      ...
      LOG.info(
          "Table {}.{} does not have an avro.schema.literal set; using Hive schema instead. "
    ...
    Schema partitionSchema =
        partitionSchema(table.getPartitionKeys(), schema, partitionColumnIdMappingString);
  ...
  private static Schema partitionSchema(
      List<FieldSchema> partitionKeys, Schema dataSchema, String idMapping) {
        ...
        f -> {
          ...
          if (field != null) {
            throw new IllegalStateException(
                String.format("Partition field %s also present in data", field.name()));
          }
          partitionFields.add(
              Types.NestedField.optional(
                  nameToId.containsKey(f.getName())
                      ? nameToId.get(f.getName())
                      : fieldId.incrementAndGet(),
                  f.getName(),
                  primitiveIcebergType(f.getType()),
                  f.getComment()));
        });
  ...
  /**
   * @param idMapping A comma separated string representation of column name and its id, e.g.
   *     partitionCol1:10,partitionCol2:11, no whitespace is allowed in the middle
   * @return The parsed in-mem Map representation of the name to id mapping
   */
  private static Map<String, Integer> parsePartitionColId(String idMapping) {
    Map<String, Integer> nameToId = Maps.newHashMap();
    if (idMapping != null) {
      // parse idMapping string
      Arrays.stream(idMapping.split(","))
          .forEach(
              kv -> {
                ...
                if (split.length != 2) {
                  throw new IllegalStateException(
                      String.format(
                          "partition.column.ids property is invalid format: %s", idMapping));
                }
                String name = split[0];
  ...
  static PartitionSpec getPartitionSpec(
      org.apache.hadoop.hive.metastore.api.Table table, Schema schema) {
  ...
  static DirectoryInfo toDirectoryInfo(org.apache.hadoop.hive.metastore.api.Table table) {
    return new DirectoryInfo(
        ...
        serdeToFileFormat(table.getSd().getSerdeInfo().getSerializationLib()),
        null);
  }
    ...
    return partitions.stream()
        .map(
            p ->
                new DirectoryInfo(
                    p.getSd().getLocation(),
                    serdeToFileFormat(p.getSd().getSerdeInfo().getSerializationLib()),
                    buildPartitionStructLike(p.getValues(), spec)))
        .collect(Collectors.toList());
  }

  private static StructLike buildPartitionStructLike(
  ...
  private static class HasDuplicateLowercaseColumnNames extends AvroSchemaVisitor<Boolean> {
    ...
    public Boolean record(
        org.apache.avro.Schema record, List<String> names, List<Boolean> fieldResults) {
