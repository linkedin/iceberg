 ...
 *
 * <p>This catalog should be only be used by metadata publishers wanting to publish/update Iceberg
 * metadata to an existing Hive table while preserving the current Hive metadata
 */
public class HiveMetadataPreservingCatalog extends HiveCatalog {
  ...
  private static final Cache<String, HiveMetadataPreservingCatalog>
      HIVE_METADATA_PRESERVING_CATALOG_CACHE = Caffeine.newBuilder().build();
  ...
  public TableOperations newTableOps(TableIdentifier tableIdentifier) {
    ...
    return new HiveMetadataPreservingTableOperations(
        conf(), clientPool(), new HadoopFileIO(conf()), name(), dbName, tableName);
  }
  ...
  public static Catalog loadHiveMetadataPreservingCatalog(String name, Configuration conf) {
    return CatalogUtil.loadCatalog(
        HiveMetadataPreservingCatalog.class.getName(), name, ImmutableMap.of(), conf);
  }
}
