...
/**
 * A {@link HiveCatalog} which uses Hive metadata to read tables. Features like time travel,
 * snapshot isolation and incremental computation are not supported along with any WRITE operations
 * to either the data or metadata.
 */
public class LegacyHiveCatalog extends HiveCatalog {
  ...
  public static Catalog loadLegacyCatalog(String name, Configuration conf) {
    return CatalogUtil.loadCatalog(
        LegacyHiveCatalog.class.getName(), name, ImmutableMap.of(), conf);
  }
  ...
  public Table loadTable(TableIdentifier identifier) {
    if (isValidIdentifier(identifier)) {
      ...
      throw new UnsupportedOperationException(
          "Metadata views not supported for Hive tables without Iceberg metadata. Table: "
              + identifier);
    } else {
