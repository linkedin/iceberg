 ...
 *
 * <p>This visitor traverses the Hive {@link TypeInfo} tree contiguously accessing the schema tree
 * for the partner schema using {@link PartnerAccessor}. When visiting each type in the Hive tree,
 * the implementation is also presented with the corresponding type from the partner schema, or else
 * a {@code null} if no match was found. Matching behavior can be controlled by implementing the
 * methods in {@link PartnerAccessor}
 *
...
public abstract class HiveSchemaWithPartnerVisitor<P, FP, R, FR> {
  ...
  /**
   * Methods to access types in the partner schema corresponding to types in the Hive schema being
   * traversed
   *
   * @param <P> type of partner schema
   * @param <FP> type of the field representation in the partner schema
  ...
  public static <P, FP, R, FR> R visit(
      TypeInfo typeInfo,
      P partner,
      HiveSchemaWithPartnerVisitor<P, FP, R, FR> visitor,
