...
public class TestLegacyHiveTableScan extends HiveMetastoreTest {
  ...
  private static final List<FieldSchema> DATA_COLUMNS =
      ImmutableList.of(
          new FieldSchema("strCol", "string", ""), new FieldSchema("intCol", "int", ""));

  private static final List<FieldSchema> PARTITION_COLUMNS =
      ImmutableList.of(
          new FieldSchema("pcol", "string", ""), new FieldSchema("pIntCol", "int", ""));

  private static final List<FieldSchema> PARTITION_COLUMNS_2 =
      ImmutableList.of(
          new FieldSchema("pcol", "string", ""),
          new FieldSchema("pIntCol", "int", ""),
          new FieldSchema("pDateCol", "date", ""));

  private static final List<FieldSchema> PARTITION_COLUMNS_3 =
      ImmutableList.of(
          new FieldSchema("pcol", "string", ""), new FieldSchema("pTsCol", "timestamp", ""));

  private static final List<FieldSchema> PARTITION_COLUMNS_4 =
      ImmutableList.of(
  ...
  public void testHiveScanSinglePartition() throws Exception {
    ...
    filesMatch(
        ImmutableMap.of("pcol=ds/pIntCol=1/B", AVRO, "pcol=ds/pIntCol=1/A", AVRO), hiveScan(table));
  }
  ...
  public void testHiveScanMultiPartition() throws Exception {
    ...
    filesMatch(
        ImmutableMap.of("pcol=ds/pIntCol=2/B", AVRO, "pcol=ds/pIntCol=1/A", AVRO), hiveScan(table));
  }
  ...
  public void testHiveScanNoAvroSchema() throws Exception {
    ...
    filesMatch(
        ImmutableMap.of("pcol=ds/pIntCol=2/B", ORC, "pcol=ds/pIntCol=1/A", ORC), hiveScan(table));
  }
  ...
  public void testHiveScanNonStringPartitionQuery() throws Exception {
    ...
    filesMatch(
        ...
        hiveScan(
            table,
            Expressions.and(Expressions.equal("pcol", "ds"), Expressions.equal("pIntCol", 1))));
  }
  ...
  public void testHiveScanComplexNonStringPartitionQuery() throws Exception {
    ...
    addPartition(
        table, ImmutableList.of("foo", 0, "a", "xy", LocalDate.of(2019, 4, 14)), AVRO, "A");
    addPartition(
        table, ImmutableList.of("foo", 1, "a", "xy", LocalDate.of(2019, 4, 14)), AVRO, "B");
    addPartition(
        table, ImmutableList.of("foo", 1, "b", "xy", LocalDate.of(2019, 4, 14)), AVRO, "C");
    addPartition(
        table, ImmutableList.of("foo", 1, "b", "xyz", LocalDate.of(2019, 4, 14)), AVRO, "D");
    addPartition(
        table, ImmutableList.of("foo", 1, "b", "xyz", LocalDate.of(2020, 4, 14)), AVRO, "E");
    addPartition(
        table, ImmutableList.of("bar", 0, "a", "xy", LocalDate.of(2020, 4, 14)), AVRO, "F");

    filesMatch(
        ImmutableMap.of(
            "pStringCol=bar/pIntCol=0/pCharCol=a/pVarcharCol=xy/pDateCol=2020-04-14/F", AVRO),
        hiveScan(table, Expressions.equal("pstringcol", "bar")));
    filesMatch(
        ImmutableMap.of(
            "pStringCol=foo/pIntCol=1/pCharCol=b/pVarcharCol=xyz/pDateCol=2019-04-14/D",
            AVRO,
            "pStringCol=foo/pIntCol=1/pCharCol=b/pVarcharCol=xyz/pDateCol=2020-04-14/E",
            AVRO),
        hiveScan(
            table,
            Expressions.and(
                Expressions.equal("pcharcol", "b"), Expressions.equal("pvarcharcol", "xyz"))));
    filesMatch(
        ImmutableMap.of(),
        hiveScan(
            table,
            Expressions.and(
                ...
                Expressions.and(
                    Expressions.equal("pcharcol", "b"), Expressions.equal("pvarcharcol", "xy")))));
  }
  ...
  public void testHiveScanHybridTable() throws Exception {
    ...
    addPartition(table, ImmutableList.of("ds", 2), ORC, "B");
    filesMatch(
        ImmutableMap.of("pcol=ds/pIntCol=1/A", AVRO, "pcol=ds/pIntCol=2/B", ORC), hiveScan(table));
  }

  private static Table createTable(
      String tableName, List<FieldSchema> columns, List<FieldSchema> partitionColumns)
      throws Exception {
  ...
  private static StorageDescriptor storageDescriptor(
      List<FieldSchema> columns, String location, FileFormat format) {
  ...
  private void addPartition(
      Table table, List<Object> partitionValues, FileFormat format, String... fileNames)
      throws Exception {
    Path partitionLocation = location(table, partitionValues);
    Files.createDirectories(partitionLocation);
    long currentTimeMillis = System.currentTimeMillis();
    metastoreClient.add_partition(
        new Partition(
  ...
  private Map<String, FileFormat> hiveScan(Table table, Expression filter) {
    ...
    CloseableIterable<FileScanTask> fileScanTasks =
        legacyCatalog
            ...
            .filter(filter)
            .planFiles();
    return StreamSupport.stream(fileScanTasks.spliterator(), false)
        .collect(
            Collectors.toMap(
                f -> {
                  String fullPath =
                      tableLocation
                          .relativize(Paths.get(URI.create(f.file().path().toString())))
                          .toString();
