...
public class TestReadFileWithCaseMismatch {
  ...
  public void writeAndValidateFileWithLowercaseFields() throws IOException {
    ...
    switch (format) {
      case AVRO:
        ...
        org.apache.avro.Schema fieldoneSchema =
            avroSchema.getField("fieldone").schema().getTypes().get(1);
  ...
  public void writeAndValidateFileWithMultipleCandidatesInSchema() throws IOException {
    ...
    switch (format) {
      case AVRO:
        ...
        org.apache.avro.Schema outerSchema =
            avroSchema.getField("outer").schema().getTypes().get(1);
      ...
      case ORC:
        TypeDescription writerSchema =
            TypeDescription.fromString("struct<outer:struct<inner:string>>");
        try (CloseableOrcWriterBatch writer = new CloseableOrcWriterBatch(testFile, writerSchema)) {
          VectorizedRowBatch batch = writer.batch;
          ((BytesColumnVector) ((StructColumnVector) batch.cols[0]).fields[0])
              .setVal(0, "1".getBytes(StandardCharsets.UTF_8));
    ...
    // When the file has a field with multiple candidates fields in the table schema, the candidate
    // which exactly
    // matches the case is selected. If no such candidate is available, the selection of candidate
    // is
    // undefined. Based on the current implementation, the first candidate in the table schema by
    // index will be picked
    // e.g. Here "inner" has two candidates in table schema "Inner" and "INNER", it will map to
    // "Inner"
    List<Record> rows = readRows(testFile, tableSchema, MappingUtil.create(tableSchema, false));
    ...
    // e.g. Here "inner" has three candidates in table schema "Inner", "INNER", and "inner", it will
    // map to "inner"
    ...
    // Even if we our read schema (projection) contains only one of the candidate fields from the
    // table schema
    ...
    rows =
        readRows(
            testFile, tableSchema.select("outer.INNER"), MappingUtil.create(tableSchema, false));
    Assert.assertNull(((Record) rows.get(0).getField("outer")).getField("INNER"));
  }
  ...
  public void writeAndValidateMultipleCandidatesInFile() throws IOException {
    ...
    switch (format) {
      ...
      case ORC:
        TypeDescription writerSchema =
            TypeDescription.fromString("struct<outer:struct<Inner:string,INNER:string>>");
        try (CloseableOrcWriterBatch writer = new CloseableOrcWriterBatch(testFile, writerSchema)) {
          VectorizedRowBatch batch = writer.batch;
          ((BytesColumnVector) ((StructColumnVector) batch.cols[0]).fields[0])
              .setVal(0, "1".getBytes(StandardCharsets.UTF_8));
    ...
    Schema tableSchema =
        new Schema(
            optional(
                1, "outer", Types.StructType.of(optional(2, "Inner", Types.StringType.get()))));
    // When the file has two fields with the same name when lowercased, both can get mapped to the
    // same field in the
    // table schema, if there isn't an exact match for both. Which of the candidates eventually gets
    // mapped is
    // undefined. e.g. here two fields "Inner" and "INNER" can be mapped insensitively to "Inner" in
    // table schema
  ...
  public void writeAndValidateDuplicateLowercaseFieldsInFile() throws IOException {
    ...
    Schema tableSchema =
        new Schema(
            optional(
                1, "outer", Types.StructType.of(optional(2, "Inner", Types.StringType.get()))));
    // When the file has two fields with the same name and case, both can get mapped to the same
    // field in the table
    // schema. Which of the candidates eventually gets mapped is undefined. Based on the current
    // implementation,
  ...
  private List<Record> readRows(File inputFile, Schema readSchema, NameMapping nameMapping)
      throws IOException {
    switch (format) {
      case AVRO:
        try (CloseableIterable<Record> reader =
            Avro.read(Files.localInput(inputFile))
                .project(readSchema)
      ...
      case ORC:
        try (CloseableIterable<Record> reader =
            ORC.read(Files.localInput(inputFile))
                .project(readSchema)
                .createReaderFunc(
                    fileSchema -> GenericOrcReader.buildReader(readSchema, fileSchema))
                .withNameMapping(nameMapping)
                .build()) {
          return Lists.newArrayList(reader);
        }
