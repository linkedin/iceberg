...
public class TestHiveSchemaConversions {
  ...
  public void testPrimitiveTypes() {
    ...
    List<PrimitiveTypeInfo> hivePrimitives =
        Lists.newArrayList(
    ...
    for (int i = 0; i < primitives.size(); i += 1) {
      ...
      Assert.assertEquals(
          "Hive schema to primitive: " + hiveType, icebergType, HiveTypeUtil.convert(hiveType));
    }
  }
  ...
  public void testConversions() {
    ...
    check(
        "struct<5: a: optional map<string, map<string, double>>>",
        "struct<a:map<string,map<string,double>>>");
    check("struct<2: l1: optional list<boolean>>", "struct<l1:array<boolean>>");
    check(
        "struct<3: l1: optional list<struct<1: l2: optional string>>>",
        "struct<l1:array<struct<l2:string>>>");
    check(
        "list<map<string, list<map<string, list<double>>>>>",
        "array<map<string,array<map<string,array<double>>>>>");
    check(
        "struct<"
    ...
    check(
        "struct<1: tag: required int, 2: field0: optional int, 3: field1: optional string>",
