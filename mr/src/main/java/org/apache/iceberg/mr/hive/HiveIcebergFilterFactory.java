...
public class HiveIcebergFilterFactory {
  ...
  // avoid it.
  private static final Class PREDICATE_LEAF_IMPL_CLASS =
      DynClasses.builder()
  ...
  private static final DynFields.UnboundField<?> LITERAL_FIELD =
      DynFields.builder().hiddenImpl(PREDICATE_LEAF_IMPL_CLASS, "literal").build();

  private static final DynFields.UnboundField<List<Object>> LITERAL_LIST_FIELD =
      DynFields.builder().hiddenImpl(PREDICATE_LEAF_IMPL_CLASS, "literalList").build();
  ...
  private static Object leafToLiteral(PredicateLeaf leaf) {
    switch (leaf.getType()) {
      ...
      case DATE:
        if (LITERAL_FIELD.get(leaf) instanceof Date) {
          return daysFromDate((Date) LITERAL_FIELD.get(leaf));
        }
        return daysFromTimestamp((Timestamp) LITERAL_FIELD.get(leaf));
      case TIMESTAMP:
