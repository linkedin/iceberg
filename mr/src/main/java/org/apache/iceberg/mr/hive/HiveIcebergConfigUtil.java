...
class HiveIcebergConfigUtil {
   ...
   *
   * <p>While copying, the schema is added into a map(tablename -> schema). This map is serialized
   * and set as the value for the key TABLE_SCHEMA_MAP.
   */
  static void copySchemaToConf(
      Supplier<Schema> schemaSupplier, Configuration configuration, Properties tblProperties) {
   ...
   *
   * <p>tblProperties is consulted to get the tablename. This tablename is looked up in the
   * serialized map present in TABLE_SCHEMA_MAP configuration. The returned json schema is then
   * parsed and returned.
   */
  static Optional<Schema> getSchemaFromConf(
      @Nullable Configuration configuration, Properties tblProperties) {
    String tableName = tblProperties.getProperty(Catalogs.NAME);
